...
<!--支付第一步-->
    <div class="firstly">
        <div class="bindmobile-wrap">
            采用<span style="color:red">微信支付</span>，购买成功后，团购券将发到您的注册邮箱：<span class="mobile">{{Auth::user()->email}}</span><a class="link"></a>
        </div>
                ...
                <tr class="j-row">
                    ...
                    <td class="j-cell">
                        <div class="buycount-ctrl">
                            ...
                            <input type="text" value="{{intval($count)}}" maxlength="10">
                            <a class="ctrl j-ctrl plus"><span class="horizontal"></span><span class="vertical"></span></a>
                        </div>
                        <span class="err-wrap j-err-wrap"></span>
                    </td>
                    <td class="price font14">¥<span class="font14 j-sumPrice">{{intval($deal->current_price)*intval($count)}}</span></td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="final-price-area">应付总额：<span class="sum">￥<span class="price">{{intval($deal->current_price)*intval($count)}}</span></span></div>
